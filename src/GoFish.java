...
public class GoFish {
    ...
    public void play() {
        // ask whether two player or single player
        System.out.println("Is this two player?: y/n");
        String onePlayer = in.nextLine().trim().toUpperCase();
        if (onePlayer.contains("Y")) {
            is2p = true;
        ...
        while (true) {
            if (whoseTurn == 'P') {
                ...
                if (computer.findAndRemoveBooks()) {
                    System.out.println(player2Name + "Oh, that's a book!");
                    if (is2p) {
                        showBooks(false);
                    }
                        else {
    ...
    private char takeTurn(boolean cpu) {
        ...
        if (!cpu) {
            if (computer.hasCard(card)) {
                System.out.println(player2Name + "Yup, here you go!");
                computer.relinquishCard(player, card);

                return 'P';
            } else {
                System.out.println(player2Name + "Nope, go fish!");
            ...
            if (player.hasCard(card)) {
                System.out.println(player2Name + "Oh, you do? Well, hand it over!");
                player.relinquishCard(computer, card);

                return 'C';
            } else {
                System.out.println(player2Name + "Ah, I guess I'll go fish...");
    ...
    private void showHand(boolean cpu) {
        if (!cpu) {
            System.out.println("\nPLAYER hand: " + player.getHand());   // only show player's hand
                ...
                System.out.println("\n" + player2Name + "books: " + computer.getHand());
        }
    }
    ...
    private void showBooks(boolean cpu) {
        if (!cpu) {
            ...
            System.out.println("\n" + player2Name + "books: " + computer.getBooks());  // shows the computer's books
        }
    }
